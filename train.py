...
for epoch in range(1, 1+num_epochs):
    ...
    for data in train_bar:
        ...
        for i, img in enumerate(fake_data[:2]):
            plot(img[0].detach().cpu().numpy(), './results/%d_%d.jpg'%(epoch-1, i), False)
    ...
    if epoch % 1 == 0:
        torch.save(netG.state_dict(), 'epochs/epoch_G_%d.pth' % epoch)
        torch.save(netD.state_dict(), 'epochs/epoch_D_%d.pth' % epoch)
